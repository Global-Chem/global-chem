<h1 align="center">Global-Chem: 常见小分子的化学知识图谱及其IUPAC/SMILES/SMARTS，用于选择与不同化学基团相关的化合物 </h1>
...
图形网络（GN）有几个规则，可以使软件工程对开发者来说更轻松。

- 必须有一个根节点。
- 当添加一个节点时，每个节点必须是连接的。
- 要删除一个节点，它必须没有任何子节点。

深度图谱网络（DGN）也有一些规则，以使更容易使用。

- 必须有一个根节点1，它标记为“输入”节点。
- 当添加一个层时，所有的节点将被添加到所有以前层的子层。(可以使用移除节点功能来执行放弃)。

### 知识图谱

没有任何依赖性，直接初始化该类就可以了! 世界上所有常见和罕见的基团都在你的掌握之中。

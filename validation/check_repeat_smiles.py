...
for new_name, new_smiles in new_molecules.items():
    print('Checking:', new_name)
    for node in nodes:
        try:
            molecules = gc.get_node_smiles(node)
        ...
        for name, smiles in molecules.items():
            ...
            if Chem.MolToSmiles(Chem.MolFromSmiles(smiles)) == Chem.MolToSmiles(Chem.MolFromSmiles(new_smiles)) and name != new_name:
                print(f'{name}: {smiles} from {node} node should be same as {new_name}: {new_smiles}')
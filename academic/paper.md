...
penalty score as an indicator of a new chemical environment that was left unaccounted for in `CGenFF`. This avoids brute force parametrization on mass molecular datasets with no clear intention and allows our force field to remain the best chemical space representation of the community with human guided direction. To demonstrate our versatility, we use our own tools to explore both avenues of explored and unexplored chemical space.
In Figure 2, we attributed the charge penalty score to an uncertainty percentage. Each molecule's top charge score was normalized against the overall top score in the common chemical universe. The more uncertain a percentage is, the more likely we are hesitant about that region of chemical space and the more worth it is navigating for force field development. Some chemical lists like Warfare, due to rare atom types not seen in CGenFF before and as such are not visible in the plot. In the chemical warfare lists, we recorded partial G, V, and A-series toxic agents according to Dr. Mirzayanov's account of the Novichok Program (80). Novichok-5 and Sarin contained fluorophosphane bonds caused by the partial charge of the phosphorous with penalties upwards of 200. 
If we investigate the medicinal chemistry (dark green) with an uncertainity percentage of nearly 40% can be attributed to emerging covalent warheads where they have gained popularity because of their unique bioactivity. Aziridine was found within this dataset and chosen because it's similar to an epoxide, acting as a good electrophilic drug fragment to extend a molecule by 2 carbons and provide a terminal reactive amine functional group for further extension. The difference in the CGenFF atom type assignment of the Aziridine is the categorical layer where the oxygen in a 3-ring membered ring system has its own specific sub-category and 3-membered ring nitrogen does not. Aziridine is assigned NG311 whereas it should be assigned NG3CG31 according to the CGenFF rules. Environment uncertain percentages were roughly in the 13% category which consists of only one sub-category, herbicides. Perfluorobutanoic acid is popular for the environmental protection agency with a unique chemical pattern where fluorine takes the place of each hydrogen along the alkyl chain, and with, typically, a tail carboxylic acid. This is not a typical feature of drug-like molecules and why we are slightly uncertain about the parameters assigned. We chose the smallest out of the dataset primarily because it was the smallest and seemed as easy to obtain parameters that best represent the dataset. In Figure 2, a major culprit to the miscellaneous 35% uncertainty position was the Vitamin C due to having high dihedral penalty scores which were unexpected initially due to its legacy as an important ubiquitous compound but a deeper look into the parameters suggests that the chemical environment, the sp2 carbon in the furanone with surrounding hydroxyls as seen in Figure 3, suggest that this would be tough to navigate through parametrization. Finally, a dithiolane fragment was discovered using a sunbursting, principal component analysis, and pdf parsing features provided in Global-Chem, when analyzing partial of the Enamine Database, and using Global-Chem's RingsInDrugs as a reference for filtering common ring systems to display emerging ring systems of interest to find **rare** viable ring systems. The 1,3-dithiolane was selected because of its simple cyclopentyl architecture but two sulphurs separated by one carbon make a great potential drug fragment anchor to enhance binding affinity due to the nucleophilicity of the sulphurs and their respective geometry. `CGenFF` did not fully recognize this ring system and was ultimately selected as the first compound for force field parametrization with the full technical account provided in the Supporting Information. 
